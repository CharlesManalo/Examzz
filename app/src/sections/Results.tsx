...
const Results = ({ result, questions, onNavigate, onReview }: ResultsProps) => {
  const getScoreMessage = () => {
    ...
    if (result.score >= 80)
      return { message: "Great job!", color: "text-blue-600", emoji: "🌟" };
    if (result.score >= 70)
      return { message: "Good work!", color: "text-violet-600", emoji: "👍" };
    if (result.score >= 60)
      return {
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-50 to-white py-8">
      <div className="container max-w-3xl mx-auto px-4">
        <Card className="border-0 shadow-xl mb-6 overflow-hidden">
          <div
            className={`bg-gradient-to-r ${getScoreColor()} p-8 text-white text-center`}
          >
          ...
          <CardContent className="p-6">
            ...
            <p className="text-center text-sm text-muted-foreground">
              {result.correctAnswers} out of {questions.length} questions
              correct
            </p>
          </CardContent>
        </Card>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-sm font-medium">
                    {Math.round(result.timeSpent / questions.length)}s per
                    question
                  </span>
                </div>
                <Progress
                  value={Math.min(
                    100,
                    (30 / (result.timeSpent / questions.length)) * 100,
                  )}
                  className="h-2"
                />
              </div>
        ...
        <Card className="border-0 shadow-md mb-6">
          <CardContent className="p-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {Array.from({
                length: questions.length - result.answers.length,
              }).map((_, index) => (
                <div
