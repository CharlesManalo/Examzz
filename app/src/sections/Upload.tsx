...
import { processDocument, isValidFileType, isValidFileSize, formatFileSize, getFileTypeLabel } from '@/services/documentProcessor';
import { generateQuiz, getQuizTitle, getRecommendedQuestionCount } from '@/services/quizGenerator';
...
const Upload = ({ user, onStartQuiz }: UploadProps) => {
  ...
  const [selectedQuizType, setSelectedQuizType] = useState<'quiz' | 'mock-exam' | 'full-exam' | 'lesson-review'>('quiz');
  const [isGenerating, setIsGenerating] = useState(false);

  const quizTypes = [
    { id: 'quiz', label: 'Quick Quiz', icon: Brain, description: '10 questions', color: 'from-blue-500 to-cyan-500' },
    { id: 'mock-exam', label: 'Mock Exam', icon: Target, description: '25 questions', color: 'from-violet-500 to-purple-500' },
    { id: 'full-exam', label: 'Full Exam', icon: Trophy, description: '50 questions', color: 'from-orange-500 to-red-500' },
    { id: 'lesson-review', label: 'Lesson Review', icon: BookOpen, description: '15 questions', color: 'from-green-500 to-emerald-500' }
  ];
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      const combinedText = completedFiles.map(f => f.extractedText).join('\n\n');
      
      const mockFile = new File([combinedText], 'combined.txt', { type: 'text/plain' });
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {quizTypes.map((type) => {
                ...
                return (
                  ...
                  >
                    <div className={`h-10 w-10 rounded-lg bg-gradient-to-br ${type.color} flex items-center justify-center mb-3`}>
          ...
          <CardContent className="p-8">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-4">
                Supported: PDF, Word, PowerPoint, Excel (Max 25MB per file, up to 15 files)
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border-0 shadow-md mb-6">
            <CardContent className="p-6">
              ...
              <div className="space-y-3">
                {files.map((file) => (
                  <div key={file.id} className="flex items-center gap-4 p-3 rounded-xl bg-muted">
